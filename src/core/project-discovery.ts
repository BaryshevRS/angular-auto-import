...
export interface AngularProjectDiscoveryOptions {
  ...
  /**
   * Directories that are Angular projects because the user said so, whatever their
   * manifest says or does not say.
   *
   * The manifest rule is a guess at where a project begins, and it is a good one for a
   * single package. It is wrong for the root of a monorepo, whose `package.json`
   * carries the build tooling while `@angular/core` is declared per application. A
   * configured `projectPath` is not a guess, so it is not re-checked: overruling an
   * explicit setting with a heuristic and then indexing nothing is the failure this
   * avoids.
   */
...
export class AngularProjectDiscovery {
  ...
  constructor(options: AngularProjectDiscoveryOptions = {}) {
    ...
    this.trustedRoots = new Set(Array.from(options.trustedRoots ?? [], (root) => path.resolve(root)));
  }
  ...
  async isAngularProject(projectRoot: string): Promise<boolean> {
    ...
    if (this.trustedRoots.has(packageRoot)) {
      return true;
    }
  ...
  /**
   * Replaces the directories believed without a manifest.
   *
   * `projectPath` can be set, corrected, and cleared while the server runs, and a root
   * that was believed only because it was named must stop being believed when it is
   * not. Nothing cached needs dropping: a trusted answer was never cached.
   * @param roots The directories to trust now.
   * @returns Whether this changed the answer for any directory. Callers act on that
   * rather than on the list of roots, because trust and roots move independently:
   * naming the folder that is already open changes nothing about *where* to look and
   * everything about whether that place is a project.
   */
  setTrustedRoots(roots: Iterable<string>): boolean {
    const replacement = new Set(Array.from(roots, (root) => path.resolve(root)));
    const unchanged =
      replacement.size === this.trustedRoots.size && [...replacement].every((root) => this.trustedRoots.has(root));
    this.trustedRoots = replacement;
    return !unchanged;
  }
