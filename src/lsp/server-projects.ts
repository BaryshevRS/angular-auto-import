...
export interface ServerProjectsOptions {
  ...
  /** Called after a root is initialized or released, so a host can report what it has. */
  onDidChangeRoots?(): void;
  logger?: CoreLogger;
}
...
export class ServerProjects {
  ...
  private readonly onDidChangeRoots: ServerProjectsOptions["onDidChangeRoots"];
  ...
  constructor(options: ServerProjectsOptions) {
    ...
    this.onDidChangeRoots = options.onDidChangeRoots;
  ...
  /**
   * Re-evaluates everything discovery decided, against a set of workspace roots.
   *
   * A root is dropped when it left the workspace, and also when it is no longer an
   * Angular project at all — which happens without anything moving on disk, when a
   * `projectPath` that was the only reason to believe it is cleared. The remaining
   * folders and open documents are then rechecked, so a project that has just become
   * one does not wait for its documents to be reopened.
   *
   * Passing the roots it already has is therefore meaningful, not a no-op: what a root
   * *is* can change while where to look does not.
  ...
  async setWorkspaceRoots(roots: Iterable<string>): Promise<void> {
    ...
    for (const knownRoot of Array.from(this.roots)) {
      const left = !findDeepestContainingProjectRoot(knownRoot, this.workspaceRoots);
      if (left || !(await this.discovery.isAngularProject(knownRoot))) {
        this.logger.info(`Releasing ${knownRoot}: it is no longer ${left ? "in the workspace" : "an Angular project"}`);
