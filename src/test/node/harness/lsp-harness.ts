...
import { PerformanceMetricsRequest, type ProjectsStatus, ProjectsStatusNotification } from "../../../lsp/protocol";
...
export interface Harness {
  ...
  /** Every projects-status the server pushed, in arrival order. */
  projectsStatuses(): ProjectsStatus[];
  /**
   * Changes what the client answers `workspace/configuration` with, and tells the
   * server the settings changed — which is what an editor does when a user edits one.
   */
  changeSettings(settings: unknown): Promise<void>;
  ...
  /**
   * Waits until the server has finished starting, whether or not it found anything.
   *
   * Discovery runs inside the `initialized` notification, which has no reply, so a test
   * that acts immediately is racing it — and a test about what a *changed* setting does
   * will otherwise pass because startup happened to read the new value. The compiler is
   * loaded after discovery, so `analysisReady` is the moment both are done.
   */
...
export async function startHarness(options: HarnessOptions = {}): Promise<Harness> {
  ...
  const projectsStatuses: ProjectsStatus[] = [];
  let refreshes = 0;

  client.onNotification(ProjectsStatusNotification, (status) => {
    projectsStatuses.push(status);
  });
  ...
  client.onRequest("workspace/configuration", (params: { items: unknown[] }) => params.items.map(() => settings ?? {}));
  ...
  return {
    ...
    async changeSettings(next: unknown) {
      settings = next;
      await client.sendNotification(DidChangeConfigurationNotification.type, {
        settings: { "angular-auto-import": next },
      });
      await settle(client);
    },
    ...
    async waitForStartup() {
      ...
      for (;;) {
        if ((await client.sendRequest(PerformanceMetricsRequest)).analysisReady) {
          return;
        }
        if (Date.now() > deadline) {
          throw new Error("Timed out waiting for the server to finish starting");
        }
