...
- `vsce:package` then `vsce:inspect`: build the VSIX and verify it carries both bundles
  and nothing they would have to resolve at runtime
...
- E2E projects live in `src/e2e/projects/` (`v19`, `v21`, `v22` — one per supported
  Angular version — plus `v22-nx`, which is there for its *layout*: manifests split the
  way a monorepo splits them, so the project root is the application and the libraries
  sit outside it. It needs no install and says so with `.e2e-no-install`.)
- The final e2e run before shipping is the v22 project in parallel mode: `pnpm run test:e2e:v22:parallel`
- And the last check of all is `pnpm run test:fixtures`: it asks the server, for every
  installed fixture project, whether it reports anything the corpus did not record. A file
  may report only when its case says `preserveImports: true`. This is what catches a false
  positive nobody's test happens to cover — the warning a maintainer would otherwise find
  by opening a fixture and seeing a squiggle.
